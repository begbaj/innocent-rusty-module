...
(@bitwise $bitwise:ident $func:ident $bitwise_assign:ident $func_assign:ident $name:ident([$t:ty]) $op:tt) => {
    impl<T: AsMut<[$t]> + AsRef<[$t]>> crate::features::utils::bitfield::ops::$bitwise for $name<T> {
        type Output = Self;
    ...
    impl<T: AsMut<[$t]> + AsRef<[$t]>> crate::features::utils::bitfield::ops::$bitwise_assign for $name<T> {
...
(@bitwise $bitwise:ident $func:ident $bitwise_assign:ident $func_assign:ident $name:ident($t:ty) $op:tt) => {
    impl crate::features::utils::bitfield::ops::$bitwise for $name {
        type Output = Self;
        fn $func(mut self, rhs: Self) -> Self {
            self.0 $op rhs.0;
            self
    ...
    impl crate::features::utils::bitfield::ops::$bitwise_assign for $name {
        fn $func_assign(&mut self, rhs: Self) {
            self.0 $op rhs.0;
        }
    }
...
(only setter; @field $(#[$attribute:meta])* ($($vis:tt)*) $t:ty, $mask:ident($mask_t:ty), $from:ty, $into:ty, _, $setter:ident: $msb:expr,
    ...
    $($vis)* fn $setter(&mut self, index: usize, value: $from) {
        use crate::features::utils::bitfield::BitRangeMut;
...
(only setter; @field $(#[$attribute:meta])* ($($vis:tt)*) $t:ty, $mask:ident($mask_t:ty), $from:ty, $into:ty, _, $setter:ident: $bit:expr) => {
    ...
    $($vis)* fn $setter(&mut self, value: bool) {
        use crate::features::utils::bitfield::BitMut;
...
(only getter; @field $(#[$attribute:meta])* ($($vis:tt)*) $t:ty, $mask:ident($mask_t:ty), $from:ty, $into:ty, $getter:ident, _: $msb:expr, $lsb:expr, $count:expr) => {
    ...
    $($vis)* fn $getter(&self, index: usize) -> $into {
        use crate::features::utils::bitfield::BitRange;
        __bitfield_debug_assert!(index < $count);
        let width = $msb - $lsb + 1;
        let lsb = $lsb + index*width;
        let msb = lsb + width - 1;
...
(only getter; @field $(#[$attribute:meta])* ($($vis:tt)*) $t:ty, $mask:ident($mask_t:ty), $from:ty, $into:ty, $getter:ident, _: $bit:expr) => {
    ...
    $($vis)* fn $getter(&self) -> bool {
        use crate::features::utils::bitfield::Bit;
        self.bit($bit)
    }
};
...
(@impl_bitrange_slice $name:ident, $slice_ty:ty, $bitrange_ty:ty) => {
    impl<T: AsRef<[$slice_ty]>> crate::features::utils::bitfield::BitRange<$bitrange_ty>
        for $name<T> {
            fn bit_range(&self, msb: usize, lsb: usize) -> $bitrange_ty {
                let bit_len = crate::features::utils::bitfield::size_of::<$slice_ty>()*8;
                let value_bit_len = crate::features::utils::bitfield::size_of::<$bitrange_ty>()*8;
                let mut value = 0;
    ...
    impl<T: AsMut<[$slice_ty]>> crate::features::utils::bitfield::BitRangeMut<$bitrange_ty>
        for $name<T> {
...
(struct $name:ident($t:ty)) => {
    impl<T> crate::features::utils::bitfield::BitRange<T> for $name where $t: crate::features::utils::bitfield::BitRange<T> {
        fn bit_range(&self, msb: usize, lsb: usize) -> T {
            self.0.bit_range(msb, lsb)
        }
    }
    impl<T> crate::features::utils::bitfield::BitRangeMut<T> for $name where $t: crate::features::utils::bitfield::BitRangeMut<T> {
...
/// ```rust
/// # #[macro_use] extern crate bitfield;
/// # use crate::features::utils::bitfield::{BitRange, BitRangeMut};
/// # fn main() {}
/// bitfield!{
///   pub struct BitField1(u16);
///   no default BitRange;
